...
Makes things possible that required a bunch of javascript in the past.
...
There's a lot more possible with this, for example a CSS-only carousel.
I'll add links to cool examples.

### 1. Scroll progress

I think this is the most common scroll indicator, a small line at the top to show how much there's left from the thing you're reading.
...
And then those keyframes connected to the progress bar element:
...
There's something spooky 🎃 going on with the last example here.
...
To customize the target we can either keep using the **anonymous** scroll progress timeline:
...
It's a significantly more complex syntax, but comes with a lot flexibility.
Super cool.
...
Let's add the timeline keyword to differentiate from
...
This might seem long, but I think in a lot of cases we would use the defaults, so we don't need all three of them.
...
And maybe a couple more, but not too many overall.
...
With Tailwind v4 moving the configuration, this is less of an issue.
Using the new keyframes is working right now with the arbitrary value syntax.
...
First of all the animation part was usually done in code and not inside the class attribute.
Maybe it was less separated (in case of `framer-motion` for example), but separated nonetheless.

The separation is pretty clear.
